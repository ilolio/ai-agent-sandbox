...
| キー | 値 | なぜ要るか |
|------|----|-----------|
| `defaultProvider` / `defaultModel` | `llamacpp` / `PI_MODEL` | pi の `--provider` 既定は `google`。向けておかないと素の `pi` が存在しないプロバイダを見に行く |
...
| `retry.provider.timeoutMs` | `1800000` | ローカル推論は遅いので 1 リクエスト 30 分まで待つ |
